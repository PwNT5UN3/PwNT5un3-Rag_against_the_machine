...
class RagAgainstTheMachine:
    ...
    def index_docs(self):
        ...
        docs_tokens = bm25s.tokenize(self.docs_doc, stopwords='en', stemmer=stemmer)
        code_tokens = bm25s.tokenize(self.docs_code, stopwords='en', stemmer=stemmer)
    ...
    def answer_question_test(self):
        ...
        print(query)
        results, scores = self.retriever_docs.retrieve(bm25s.tokenize(query), k=5)
        results2, scores2 = self.retriever_code.retrieve(bm25s.tokenize(query), k=5)
        retrieved_doc = []
        retrieved_code = []
        ...
        context_docs = [([self.docs_doc[doc], self.metadata_doc[doc]]) for doc, _ in retrieved_doc]
        context_code = [([self.docs_code[doc], self.metadata_code[doc]]) for doc, _ in retrieved_code]
        print("Retrieved:")
